...
In order to protect validators from downgrade attacks, this document mandates the use of DNSKEY-ADT flag described in {{!I-D.ietf-dnsop-delext}}.
...
{{!I-D.ietf-dnsop-delext}} section 4 applies.
...
For a query of the delegated zone itself, the response has an RCODE of NOERROR; for a query that has more labels than the delegated zone, the response has an RCODE of NXDOMAIN; this is no different than the algorithm applicable to DELEG-unaware authoritative servers originally defined in {{!RFC1034}} (and later updated).
